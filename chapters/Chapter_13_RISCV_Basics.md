...
আমরা বানাব শুধু **RV32I** — মানে base 32-bit integer set, কোনো extension ছাড়া। শুনতে সীমিত লাগলেও এটা একটা সম্পূর্ণ, কাজ-করা Turing-complete প্রসেসর — এতেই loop, function, recursion, এমনকি একটা ছোট OS পর্যন্ত চলে। গুণ-ভাগ (M) না থাকলে সেগুলো add আর shift দিয়ে software-এ করে নেওয়া যায়। শেখার জন্য এটাই perfect: যথেষ্ট ছোট যে পুরোটা বানানো যায়, আবার যথেষ্ট আসল যে গর্ব করা যায়।
...
- **Modularity (মডুলারিটি):** base + extension — যেটা আগেই দেখেছ। তুমি শুধু যা ব্যবহার করবে তার জন্যই transistor খরচ করবে।
...
- **বিশেষ কাজের register (x0–x4):** `zero` সবসময় ০, `ra` function থেকে ফেরার ঠিকানা রাখে, `sp` stack-এর মাথা দেখায়, `gp`/`tp` global ও thread data-র দিকে নির্দেশ করে।
...
এখানে একটা জরুরি ধারণা: **sign-extension**। immediate মাত্র ১২ বিট, কিন্তু হিসাব হয় ৩২ বিটে। তাহলে বাকি ২০ বিট কী দিয়ে ভরবে? শুধু ০ দিয়ে ভরলে ঋণাত্মক সংখ্যা নষ্ট হয়ে যাবে। তাই RISC-V immediate-এর সবচেয়ে বাঁ দিকের বিট (চিহ্ন-বিট, বিট ১১) **নকল করে** বাকি ২০ বিট ভরে। ফলে `-5` (যার চিহ্ন-বিট ১) ৩২ বিটেও `-5` থাকে, আর `+10` (চিহ্ন-বিট ০) `+10`-ই থাকে। এজন্যই `SUBI` নামে কোনো instruction লাগে না — `ADDI x5, x5, -5` দিয়েই বিয়োগ হয়ে যায়, কারণ immediate ঋণাত্মক হতে পারে।
...
> 💡 **মনে রেখো:** B-type-এ বিট ৭ ধরে রাখে `imm[11]`, আর `imm[0]` কখনো জমা থাকে না — সেটা সবসময় ০ (implicit, halfword aligned)। তাই reconstruction-এ সবচেয়ে নিচে একটা ০ বসিয়ে দিলেই আসল ১৩-বিট offset (±4KB) তৈরি হয়।
...
দুটো জিনিস বুঝে রাখো। প্রথমত, **logical (SRL) বনাম arithmetic (SRA) ডান-shift**: SRL উপরে ০ ভরে, কিন্তু SRA চিহ্ন-বিট নকল করে উপরে ভরে — যাতে ঋণাত্মক সংখ্যা ডানে সরালেও ঋণাত্মকই থাকে (অর্থাৎ চিহ্নসহ ভাগ ঠিক থাকে)। বাঁ-shift-এ এই ঝামেলা নেই, তাই SLL একটাই। দ্বিতীয়ত, shift amount কেন মাত্র **৫ বিট (০–৩১)**? কারণ ৩২-বিট register-কে ৩২ বা তার বেশি ঘর সরানোর কোনো মানে নেই (সব বিট বেরিয়ে যাবে), আর ০–৩১ ঠিক ৫ বিটেই ধরে।
...
কয়েকটা জায়গা একটু খুলে বলি। শুরুতে `main` কেন `ra` stack-এ রাখছে? কারণ `main` নিজেই হয়তো অন্য কারো ডাকা function — তার `ra`-তে main-এর ফেরার ঠিকানা আছে। এখন `jal add_func` করলে সেই `ra` চাপা পড়ে যাবে! তাই call-এর আগে পুরোনো `ra` stack-এ জমিয়ে রাখা, আর call-এর পরে আবার তুলে আনা। `addi sp, sp, -16` দিয়ে stack-এ ১৬ বাইট জায়গা করা হয় (stack নিচের দিকে বাড়ে, তাই বিয়োগ; RISC-V ABI অনুযায়ী `sp` সবসময় ১৬ বাইটে aligned থাকতে হয়, তাই মাত্র ৪ বাইট দরকার হলেও ১৬ নেওয়া হয়), শেষে `addi sp, sp, 16` দিয়ে সেই জায়গা ফেরত দেওয়া হয় — না দিলে stack ধীরে ধীরে উপচে পড়বে (stack overflow!)। লক্ষ করো `add_func` নিজে কোনো function ডাকে না, তাই তার `ra` নিরাপদ — তার stack ছোঁয়ার দরকারই হয়নি।

### Register Usage:

পুরো চুক্তিটা এক জায়গায় গুছিয়ে রাখলাম — function লেখার সময় এটাই তোমার রেফারেন্স:
...
এখন section ১৩.২-এর caller-saved বনাম callee-saved-এর ব্যবহারিক মানে পরিষ্কার হবে। তোমার function-এর ভেতরে যদি এমন কোনো মান লাগে যা অন্য function ডাকার পরেও টিকে থাকতে হবে — সেটা `s` register-এ রাখো (callee-saved, call টিকে যায়), কিন্তু ব্যবহারের আগে তার পুরোনো মান stack-এ জমিয়ে শেষে ফেরত দিতে হবে। আর ক্ষণিকের হিসাবের জন্য `t` register ব্যবহার করো (caller-saved, save করার ঝামেলা নেই, তবে অন্য function ডাকলে এর মান হারিয়ে যেতে পারে)। সংক্ষেপে: **টিকে থাকা মান → `s`, ক্ষণিকের মান → `t`।**

---

## ১৩.৭ Your 1-Week Build Plan

এই chapter-এ তুমি ISA-টা বুঝলে — পরের chapter-এ এটাকে সিলিকনে রূপ দেবে। মাঝের এই সপ্তাহটা সেই সেতু। প্রতিদিন একটা করে component-এ মনোযোগ দাও; দিনের শেষে ছোট ছোট জিনিস কাজ করতে দেখলে আত্মবিশ্বাস বাড়বে, আর Chapter 14-এ পুরো CPU জোড়া লাগানোর সময় প্রতিটা টুকরো তোমার চেনা থাকবে। তাড়াহুড়ো নয় — বুঝে এগোও।
